...
class CreateAdminProfilePage extends StatefulWidget {
  @override
  _CreateAdminProfilePageState createState() => _CreateAdminProfilePageState();
}

class _CreateAdminProfilePageState extends State<CreateAdminProfilePage> {
  ...
  Future<void> createAdminProfile() async {
    ...
    try {
      ...
      DocumentSnapshot phoneCheck =
          await _firestore.collection('admins').doc(phone).get();
  ...
  Future<void> confirmOTP(ConfirmationResult confirmationResult, String otp, String name, String username, String phone, String password) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Admin profile created successfully')),
      );
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => LoginPage(isAdmin: true),
        ),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
